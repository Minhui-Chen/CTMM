...
sns.lineplot(x='noise', y='power', hue='Method', palette=colors[1:4], style='Method', 
        dashes=False, markers=True, markeredgecolor='None', data=data, ax=axes[0], lw=lw)
axes[0].set_xlabel(r'Coefficient of variation($\nu$)', fontsize=7)
axes[0].set_ylabel('False positive rate', fontsize=7)
axes[0].text(-0.05, 1.05, '(a)', fontsize=7, transform=axes[0].transAxes)
...
sns.lineplot(x='V', y='power', hue='Method', palette=colors[1:4], style='Method', 
        dashes=False, markers=True, markeredgecolor='None', data=data3, ax=axes[1], lw=lw)
axes[1].set_ylabel('True positive rate', fontsize=7)
axes[1].axhline(y=0.05, color='0.8', ls='--', zorder=0)
axes[1].text(-0.05, 1.05, '(b)', fontsize=7, transform=axes[1].transAxes)
