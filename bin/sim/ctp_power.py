...
def main():
    ...
    ## cell type number
    hom_c = [np.load(f, allow_pickle=True).item() for f in snakemake.input.hom_c]
    print(snakemake.input.hom_c)
    hom_c_remlJK = [np.load(f, allow_pickle=True).item() for f in snakemake.input.hom_c_remlJK]
    free_c = [np.load(f, allow_pickle=True).item() for f in snakemake.input.free_c]
    free_c_remlJK = [np.load(f, allow_pickle=True).item() for f in snakemake.input.free_c_remlJK]
    hom_c_labels = snakemake.params.hom_c
    print(hom_c_labels)
    free_c_labels = snakemake.params.free_c
    ...
    # cell type number
    ml_hom_c_power = collect_power(hom_c, hom_c_labels, snakemake.params.arg_c, 'ml')
    ml_free_c_power = collect_power(free_c, free_c_labels, snakemake.params.arg_c, 'ml')
    reml_hom_c_power = collect_power(hom_c, hom_c_labels, snakemake.params.arg_c, 'reml')
    reml_free_c_power = collect_power(free_c, free_c_labels, snakemake.params.arg_c, 'reml')
    remlJK_hom_c_power = get_power(hom_c_remlJK, hom_c_labels, snakemake.params.arg_c, 'reml', 'wald')
    remlJK_hom_c_power['Method'] = 'REML (JK)'
    remlJK_hom_c_power['Test'] = 'Wald'
    remlJK_free_c_power = get_power(free_c_remlJK, free_c_labels, snakemake.params.arg_c, 'reml', 'wald')
    remlJK_free_c_power['Method'] = 'REML (JK)'
    remlJK_free_c_power['Test'] = 'Wald'
    he_hom_c_power = get_power(hom_c, hom_c_labels, snakemake.params.arg_c, 'he', 'wald')
    he_free_c_power = get_power(free_c, free_c_labels, snakemake.params.arg_c, 'he', 'wald')
    he_hom_c_power['Test'] = 'Wald'
    he_free_c_power['Test'] = 'Wald'

    hom_c_power = pd.concat([ml_hom_c_power, reml_hom_c_power, remlJK_hom_c_power, he_hom_c_power], 
            ignore_index=True)
    free_c_power = pd.concat([ml_free_c_power, reml_free_c_power, remlJK_free_c_power, he_free_c_power], 
            ignore_index=True)
    ...
    # plot
    mpl.rcParams.update({'font.size': 7, 'lines.markersize': mpl.rcParams['lines.markersize']*0.8})
    fig, axes = plt.subplots(nrows=4, ncols=2, sharex='row', sharey=False, figsize=(6.85,8), dpi=600)
    alpha = 1
    lw = 1
    #ms = 10
    ...
    ## cell type number
    sns.lineplot(data=hom_c_power, x='label', y='Positive rate', hue='Method', 
            style='Test', dashes={'Wald':'','LRT':(2.5,2)}, ax=axes[2,0], markers=True, alpha=alpha, lw=lw)
    sns.lineplot(data=free_c_power, x='label', y='Positive rate', hue='Method', 
            style='Test', dashes={'Wald':'','LRT':(2.5,2)}, ax=axes[2,1], markers=True, alpha=alpha, lw=lw)
    axes[2,0].set_xlabel('Number of cell types', fontsize=10)
    axes[2,1].set_xlabel('Number of cell types', fontsize=10)
    axes[2,0].axvline(x=0, color='0.8', ls=':', zorder=0)
    axes[2,1].axvline(x=0, color='0.8', ls=':', zorder=0)

    ## variance component
    sns.lineplot(data=free_vc_power, x='label', y='Positive rate', hue='Method', 
            style='Test', dashes={'Wald':'','LRT':(2.5,2)}, ax=axes[3,1], markers=True, alpha=alpha, lw=lw)
    axes[3,1].set_xlabel(r'$\bar{V}$', fontsize=10)
    axes[3,1].axvline(x=2, color='0.8', ls=':', zorder=0)
