...
def get_power(outs, labels, arg, method):
    ...
    plot_order = np.array(snakemake.params.plot_order)
    try:
...
def main():
    ...
    reml_jk_ctp_hom_power = get_power(ctp_hom_remlJK, hom_labels, 'ss', 'REML (Wald)')
    ...
    reml_hom_power = pd.concat( [
        reml_jk_ctp_hom_power, reml_lrt_ctp_hom_power, 
        he_ctp_hom_power], 
        ignore_index=True )
    ...
    reml_hom_power = reml_hom_power.rename( columns={'label':'Number of cell types'} )

    reml_jk_ctp_free_power = get_power(ctp_free_remlJK, free_labels, 'ss', 'REML (Wald)')
    ...
    reml_free_power = pd.concat( [
        reml_jk_ctp_free_power, reml_lrt_ctp_free_power, he_ctp_free_power], 
        ignore_index=True )
    ...
    reml_free_power = reml_free_power.rename( columns={'label':'Number of cell types'} )
    ...
    sns.lineplot(data=reml_hom_power, x='Number of cell types', y='Positive rate', hue='Method', 
            alpha=alpha, lw=lw, legend=False, ax=axes[0], palette=[colors[2], colors[1], colors[3]], marker='o')
    ...
    axes[0].set_xlabel('Number of cell types', fontsize=9)
    ...
    first_legend = axes[0].legend(handles=[line2, line3, line4], loc='upper right', title='Method')
    ...
    sns.lineplot(data=reml_free_power, x='Number of cell types', y='Positive rate', hue='Method', 
            alpha=alpha, lw=lw, ax=axes[1], palette=[colors[2], colors[1], colors[3]], marker='o')
            # style='Framework', dashes={'OP':(3,2), 'CTP':''}, ax=axes[1], markers={'OP':'X', 'CTP':'o'}, 
    axes[1].set_ylabel('True positive rate', fontsize=9)
    axes[1].set_xlabel('Number of cell types', fontsize=9)
