...
def null_ML(y_f, P_f, nu_f):
    print('Null ML', time.time())

    def null_ml_subprocess(y_f, P_f, nu_f, out={}):
        ctng_ml_rf = 'bin/ctng.ml.fast.R'
        ctng_ml_r = STAP( open(ctng_ml_rf).read(), 'ctng_ml_r' )
        out_ = ctng_ml_r.screml_null(Y=r['as.matrix'](r['read.table'](y_f)), P=r['as.matrix'](r['read.table'](P_f)),
                vs=r['as.matrix'](r['read.table'](nu_f)))
        for key, value in zip(out_.names, list(out_)):
            out[key] = value

    out = subprocess_run(null_ml_subprocess, args=(y_f, P_f, nu_f))

    beta, l, hess = np.array(out['beta']), out['l'][0], np.array(out['hess'])
    return({'beta':{'ct_beta':beta}, 'l':l, 'hess':hess})
...
#
#        hom2 = vars['mt1'] / ( (N-1) * C * C )
#
#        # beta
#        A = np.ones((C,C)) * hom2
#        sig2s = np.kron(np.eye(N), A) + np.diag(vs.flatten())
...
#    n_par = 1
#
#    hom2, beta = hom_HE_(Y, vs)
#
#    ### jackknife
#    jacks = [hom_HE_(np.delete(Y, i, axis=0), np.delete(vs, i, axis=0)) for i in range(Y.shape[0])]
...
#    he = {'hom2': hom2, 'ct_beta':beta}
#    p = {'hom2': wald.wald_test(hom2, 0, var_hom2),
#            'ct_beta': util.wald_ct_beta(beta, var_beta, n=N*C, P=n_par+C)}
#
...
def iid_ML(y_f, P_f, ctnu_f, nu_f=None, fixed_covars_d={}, random_covars_d={}, par=None, nrep=10):
    ...
    def iid_ml_subprocess(Y, P, ctnu, fixed_covars_array_d, random_covars_array_d, par, nrep, out):
        ctng_ml_rf = 'bin/ctng.ml.fast.R'
        ctng_ml_r = STAP( open(ctng_ml_rf).read(), 'ctng_ml_r' )
        numpy2ri.activate()
        if par is None:
    ...
    out = subprocess_run(iid_ml_subprocess, 
            args=(Y, P, vs, fixed_covars_array_d, random_covars_array_d, par, nrep))
    ...
    Vy = np.kron(np.eye(N), A) + np.diag(vs.flatten())
    Z = [np.kron(np.eye(N), np.ones((C,1))), np.eye(N*C)]
    if len(r2_d.keys()) > 0:
        for key in np.sort( list(r2_d.keys()) ):
            M_ = np.kron( random_covars_array_d[key], np.ones((C,1)) )
            Vy = Vy + r2_d[key] * M_ @ M_.T
...
def iid_reml_subprocess(Y, P, ctnu, fixed_covars_array_d, random_covars_array_d, par, nrep, out={}):
    ctng_reml_rf = 'bin/ctng.reml.fast.R'
    ctng_reml_r = STAP( open(ctng_reml_rf).read(), 'ctng_reml_r' )
    numpy2ri.activate()
    if par is None:
...
def iid_REML(y_f, P_f, ctnu_f, nu_f=None, fixed_covars_d={}, random_covars_d={}, par=None, nrep=10, 
    ...
    #
    out = subprocess_run(iid_reml_subprocess, 
            args=(Y, P, vs, fixed_covars_array_d, random_covars_array_d, par, nrep))
    ...
    # wald
    A = np.ones((C,C))*hom2+V
    Vy = np.kron(np.identity(N), A) + np.diag(vs.flatten())
    Z = [np.kron(np.eye(N), np.ones((C,1))), np.eye(N*C)]
    if len(r2_d.keys()) > 0:
        for key in np.sort( list(r2_d.keys()) ):
            M_ = np.kron( random_covars_array_d[key], np.ones((C,1)) )
    ...
    if not jack_knife:
        ...
        pool = multiprocessing.Pool( processes=4 )
        pool_out = pool.starmap(iid_reml_subprocess, args)
        pool.close()
        pool.join()
...
#        # t = \ve{ y' y'^T - \Pi_P^\perp D \Pi_P^\perp }
#        mt2 = (vars['y_p']**2).sum() - vs.sum() * (N-1) / N
#
#        #
#        mt = np.array([vars['mt1'], mt2])
#
...
#    n_par = 1 + 1
#
#    theta, beta = iid_HE_(Y, vs)
#
#    ### jackknife
#    jacks = [iid_HE_(np.delete(Y, i, axis=0), np.delete(vs, i, axis=0)) for i in range(N)]
#    jacks_hom2 = [x[0][0] for x in jacks]
#    var_hom2 = (len(jacks) - 1.0) * np.var(jacks_hom2)
#    jacks_het = [x[0][1] for x in jacks]
#    var_het = (len(jacks) - 1.0) * np.var(jacks_het)
#    jacks_beta = [x[1] for x in jacks]
#    var_beta = (len(jacks) - 1.0) * np.cov(np.array(jacks_beta).T, bias=True)
#
#    he = {'hom2': theta[0], 'V': np.eye(C) * theta[1]}
#    p = { 'hom2': wald.wald_test(theta[0], 0, var_hom2),
#            'V': wald.wald_test(theta[1], 0, var_het),
#            'ct_beta': util.wald_ct_beta(beta, var_beta, n=N*C, P=n_par+C) }
#
#    print( time.time() - start )
#    return(he, p)
...
#    n_par = 1 + C 
#
#    theta, beta = free_HE_(Y, vs)
#    hom2, V = theta[0], np.diag(theta[1:])
#
#    ### jackknife
#    jacks = [free_HE_(np.delete(Y, i, axis=0), np.delete(vs, i, axis=0)) for i in range(N)]
#    jacks_hom2 = [x[0][0] for x in jacks]
#    var_hom2 = (len(jacks) - 1.0) * np.var(jacks_hom2)
...
#
#    he = {'hom2': hom2, 'V': V}
#    p = {'hom2': wald.wald_test(hom2, 0, var_hom2, N*C-n_par),
#            'V': wald.mvwald_test(np.diag(V), np.zeros(C), var_V, n=N*C*N*C, P=n_par),
#            'Vi': [wald.wald_test(V[i,i], 0, var_V[i,i], N*C-n_par) for i in range(C)],
#            'ct_beta': util.wald_ct_beta(beta, var_beta, n=N*C, P=n_par+C) }
#
#    print( time.time() - start )
#    return(he, p)
